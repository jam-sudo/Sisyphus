...
4-track, regenerated **2026-05-01** post-Hardening (predict deterministic path uses `BodyGraph.realize_means()` instead of `graph.sample(rng=42)`, restoring pre-Achour 2026-04-14 canonical baseline by eliminating seed-dependent RNG-order coupling). N=107 holdout via `scripts/run_engine_benchmark.py` → `data/training/4track_holdout_predictions.json`. 95% CIs bootstrap (10k resamples, seed=20260422). CI artifact: `data/validation/4track_ci_2026-05-01.json`.

| Track | AAFE | 95% CI | %2-fold | %3-fold | N |
|---|---|---|---|---|---|
| **Meta (production)** | **2.695** | [2.31, 3.16] | 46.7 | 63.6 | 107 |
| Engine only | 3.757 | [3.13, 4.52] | 32.7 | 43.9 | 107 |
| ML only | 3.057 | [2.59, 3.62] | 42.1 | 57.0 | 107 |
| In-domain Meta | 2.732 | [2.29, 3.27] | 46.2 | 62.5 | 80† |
...
- **Restores pre-Achour canonical baseline**: Meta 2.702 → **2.695** (= 2026-04-14 historical value); the prior 2026-04-29 → 2026-04-30 → 2026-05-01 cache values were all RNG-order artifacts of seed=42 lognormal realization, not real engine drift.
- Engine track shifted 3.572 → 3.757 (within Engine CI [3.13, 4.52]). Engine track was previously enjoying seed=42-favorable realization; mean-only is the canonical deterministic central tendency.
- ML AAFE invariant (3.057, ML model artifacts unchanged).
- propranolol engine_validation xfail (~16% drift) flipped to PASS — same RNG mechanism.
...
- 107-holdout AAFE bit-identical pre-vs-post v3 (predict invariance confirmed by §6.2 leak audit; 4 prodrugs absent from holdout).
- 4 prodrug 3-fold gates remain xfail per spec §6.4 (extraction-step rate-limits dominate over active CL/V disposition).
