...
|---|:-:|:-:|:-:|:-:|:-:|
| **Meta-learner (production)** | **2.753** | [2.36, 3.23] | 44.9% | 64.5% | 107 |
| Engine only | 4.010 | [3.36, 4.82] | 29.0% | 46.7% | 107 |
| ML only | 3.010 | [2.56, 3.56] | 43.0% | 57.9% | 107 |
| Meta, in-domain | 2.838 | [2.37, 3.42] | 42.0% | 61.7% | 81 |

> **Reproducibility note (2026-05-09 audit-driven update; B-03 refresh 2026-05-20).** These numbers reflect a **public-clone deterministic state**: a fresh `git clone` followed by `pip install -r requirements-lock.txt` reproduces the headline AAFE to 3 sig figs (independently re-measured 2026-05-19 on a parallel Python 3.13 / macOS-arm64 / brew libomp stack: Meta 2.755 vs 2.751, Engine 4.015 vs 4.008, ML 3.010 vs 3.012, in-domain Meta 2.840 vs 2.837, in-domain N=81 ✓). Per-drug Cmax bit-identity additionally requires matching the exact numerics stack (Python minor version, BLAS implementation, libomp build) used to generate the canonical `data/training/4track_holdout_predictions.json`; on differing environments per-drug Cmax can drift up to ~12% on individual drugs (largest observed: trazodone engine 0.291 → 0.256) without materially shifting aggregate AAFE. The previous cache (Meta 2.679 [2.30, 3.14], In-domain 2.733) was generated on a local-developer environment that conditionally loaded two artifacts not present in this repository: a proprietary DrugBank export (`data/drugbank/`, academic license required, gitignored) and a residual-correction logP XGBoost model (`models/adme/logp_correction.json`, gitignored). Both shifted Cmax predictions silently for the drugs they covered. Removing the silent shift moves Meta AAFE from local-artifact values near 2.68 to the public-clone 2.75 range. Bootstrap 95% CIs (10,000 resamples on |log<sub>10</sub>(fold)|, seed=20260422, computed 2026-05-20 after B-03) are above; the refreshed Meta CI [2.36, 3.23] overlaps the previous [2.30, 3.14], confirming the artifact shift remains within sampling uncertainty. Prospective slice (15 NMEs) refresh is deferred to a follow-up commit (small expected delta; only ~3 of 15 drugs have DrugBank records). Artifact: `data/validation/4track_ci_2026-05-12_v0.4.json`.
