...
## 2026-06-04 — FLUX-1: flow-limitation double-count fix (DE-41/42/43 root cause) — correct physics, headline REGRESSES (pending canonical regen)
...
**Stack + canonical-regen handoff.** This dev machine is macOS/Accelerate/py3.13; CI is Linux/OpenBLAS/py3.10 — ~2.3% aggregate drift (my public-clone pre-fix = 2.762 vs canonical cache = 2.698), no docker. So the committed cache is left at the **canonical pre-FLUX-1 value (2.698)**; the post-fix cache + goldens + cache-pin + CI bootstrap must be regenerated in the locked-requirements Linux env. Stale-pending-regen tests are **xfailed** (clear FLUX-1 reasons): `test_oatp_ecm_statins[pravastatin,pitavastatin]`, `test_predict_auto_ecm[pravastatin,pitavastatin]` (OATP1B1 abundance was calibrated against the wrap; re-anchor needs a non-holdout OATP1B1 substrate — pravastatin is holdout), `test_ecm_holdout_spot_check_10_drugs` (stale cache), `test_enzyme_leak_audit` (stale `prodrug_v3_pre_baseline.json` — FLUX-1 is an intended global change), `test_cmax_snapshot[tebipenem_pivoxil]` (stale pin). To complete (one command): in the canonical Linux env, hide the artifacts then run **`PYTHONPATH=src python scripts/regen_flux1_canonical.py`** — it regenerates the cache + leak-audit baseline + CI bootstrap and prints the exact test-edit values (cache-pin AAFE, tebipenem `_PINNED`) and the xfails to remove (the cache/baseline ones; KEEP the OATP ones). Then update the CLAUDE.md headline table to the new Meta, restore artifacts, run the full suite, commit, merge. **Follow-up:** re-anchor OATP1B1 abundance to a non-holdout substrate (rosuvastatin/pitavastatin) to un-xfail the statin/auto_ecm tests.
