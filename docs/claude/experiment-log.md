...
## 2026-06-04 — FLUX-1: flow-limitation double-count fix (DE-41/42/43 root cause) — Meta 2.698 → 2.625
...
**Outcome (correctness-first, honest report; this environment reproduces the 2.698 baseline exactly).** Holdout Meta **2.698 → 2.625** (−2.7%, %3-fold 61.7→63.6), in-domain **2.760 → 2.667** (−3.4%). Engine track **3.831 → 4.488** (+17%, WORSE) — this is *exposed error-cancellation*: high-first-pass actives correct toward observed (selegiline 20.5×→5.1× over, methylphenidate 9.8×→2.9×, oxybutynin 4.0×→0.7×), while drugs already under-predicted for unrelated reasons (ranolazine — solubility-limited absorption; the xfail ester prodrugs tenofovir-DF/valganciclovir/valacyclovir) get worse because the under-extraction bug was offsetting those errors. Prospective N=28 Meta **3.21 → 3.27** (slightly worse, within CI [2.42,4.37]); %2-fold 25.0→28.6. CI refreshed `data/validation/4track_ci_2026-06-03_flux1.json` (Meta [2.26, 3.08]).
...
**Test triage (12 failures, all resolved; suite 903 passed / 4 xfailed / 0 failed).** Formula-encoding unit tests (`test_ecm_flux` ×3, `test_prodrug_v2_flux`, `test_prodrug_v2_mass_balance`, `test_flux_fu_correction` ×3) updated from the whole-organ wrap to the intrinsic clearance. Omega-parity goldens midazolam 0.006943→0.005909, propranolol 0.1355→0.082528 (predecessor shared the double-count; caffeine/warfarin low-extraction, unchanged). `test_tdm_enkf[morphine]` stale precondition updated (mechanism intact). **`test_oatp_ecm_statins[pravastatin]` xfailed** (FE 1.07→2.28): the ECM fix exposed that OATP1B1 abundance (5e5) was calibrated against the wrap; re-anchor needs a *non-holdout* OATP1B1 substrate (pravastatin is holdout) — deferred follow-up.
