...
- **+BSA / albumin-mediated PSu,inf (OATP)** — *keep as correctness only, not a lever*: a real primary-literature mechanistic fix (Li/Benet 2020, ~1.9–2.0 fold, no scaling) but **inert** — touches 1 holdout drug (pravastatin, inviolable) and 0 prospective → ΔMeta≈0.000. Ships under correctness-over-benchmark (spec `2026-06-04-oatp-ecm-reanchor-design.md`). **Status 2026-07-06:** the abundance re-anchor (off holdout pravastatin → non-holdout pitavastatin) and the auto-ECM xfail closure shipped (statin FE-gates green, #103); the **+BSA PS reparameterization itself is formally DEFERRED as data-blocked** — the per-substrate PSu,inf(+BSA)/Kp values live only in the paywalled Li/Benet 2020 primary source (AAPS J; not to be fabricated), and even implemented it is near-zero-*observable* (the abundance re-anchors to pitavastatin, so only *relative* statin PS shifts; pravastatin stays validation-only) on top of headline-inert. Re-open only if the primary-source table becomes available.
