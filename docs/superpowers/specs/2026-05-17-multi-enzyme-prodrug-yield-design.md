...
1. **Register only CYP2C19 with yield=0.15.** Loses the CES1 dead-end branch entirely; parent CL is 5–7× under-clear (engine path zeroed for double-count avoidance via `metabolic_fraction=0`, no CES1 ProdrugActivationEdge to compensate). Parent AUC 5–7× over → active production proportionally over.
2. **Register both CES1 and CYP2C19, single entry-level yield (e.g., 0.15).** Engine applies yield=0.15 to *both* edges' fluxes. CES1 then "produces" 15% × 0.85 = 12.75% of dose as active R-130964 — biologically wrong (CES1 makes SR26334, not R-130964). Violates mass balance against measured ~3-5 ng/mL clinical R-130964 plasma.
3. **Register both with yield=0.85 (CES1's share).** Symmetric error — CYP2C19 now produces 85% × 0.15 = 12.75%, still biologically wrong because the routing-by-enzyme is gone.
...
2. Registry entry with `observation_species="parent"` (107-holdout reference is parent clopidogrel Cmax; mismatch with "active" would inject a deliberate 5-20× species-mismatch fold error).
