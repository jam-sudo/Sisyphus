...
- **Track 2 — measured-input engine path.** An *additive, bit-identical-when-unused* route that feeds measured ADME into the engine. Consumed via `result.engine_pk` / an `--engine-only` benchmark, it bypasses the meta-learner. Reproduced headroom (2026-06-02, `run_measured_adme_benchmark.py`): clean-10 SMILES **2.63 → measured 2.33** (~11%). **Caveat (2026-06-02, supersedes v2 optimism):** the engine-only path is *not* error-cancellation-free — the engine carries internal structural error that correct ADME can *expose* (alprazolam FE 2.67 → 6.04 under correct fup), so it is a **probe of engine structure**, not a guaranteed clean win. The legacy "1.980" was a stale engine state (see diagnosis.md §3).
...
- **Pattern C (reconciled 2026-06-02):** production engine-only clean-10 SMILES 2.63 → measured **2.33**; the legacy 1.980 was a stale engine state. Floor is engine *structural* error (not the CLint R²=0.24 noise floor), and correct ADME can *expose* it (alprazolam worsens) — the path is not fully error-cancellation-free.
