#!/usr/bin/env python3
"""Train a conditional multi-drug NPE from multi_drug_*.npz data.

The amortizer learns ``p(theta | x_obs)`` where ``x_obs`` is the
concatenation of:
    [log10(Cmax), drug_features (12-D)]

so the density estimator conditions on both the observed Cmax and the
nominal ADME profile of the drug. One network serves all drugs in the
training set.

Usage::

    python scripts/sbi_train_multi_drug.py \\
        --data data/sbi/multi_drug_mini.npz \\
        --out  models/sbi/multi_drug_mini_nsf.pt \\
...
log = logging.getLogger("sbi_train_md")
...
def main() -> None:
    ...
    log.info(
        "  theta=%s  log10_cmax=%s  drug_features=%s",
        theta.shape,
        log10_cmax.shape,
        drug_features.shape,
    )
    log.info(
        ...
        float(log10_cmax.min()),
        float(log10_cmax.max()),
    )

    # Pack observation: (N, 1 + 12) = (N, 13)
    x_full = np.concatenate([log10_cmax, drug_features], axis=1).astype(np.float32)
    log.info("  x_full (packed) shape=%s", x_full.shape)

    # Standardize drug-feature columns only (log10_cmax is already in log-space
    # and on a consistent scale; mixing standardized and unstandardized is safer
    # than rescaling the observation that maps to the physical assay).
    n_features = drug_features.shape[1]
    feat_mean = drug_features.mean(axis=0)
    feat_std = drug_features.std(axis=0)
    feat_std[feat_std < 1e-6] = 1.0
    x_full[:, 1 : 1 + n_features] = (drug_features - feat_mean) / feat_std
    ...
    # Persist feature standardizer alongside the posterior so SBC/inference
    # scripts apply the identical transform.
